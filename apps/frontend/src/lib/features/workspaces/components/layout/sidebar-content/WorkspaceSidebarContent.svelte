...
<script lang="ts">
			...
			items: [
				{
					...
					href: resolve('/dashboard/workspaces/[slug]', { slug }),
					matchPrefix: false,
					color: undefined
				},
				{
					...
					title: 'MyTasks',
					icon: ClipboardCheck,
					href: resolve('/dashboard/workspaces/[slug]/tasks', { slug }),
					matchPrefix: false,
					color: undefined
				}
			]
			...
			const items =
				section.id === 'projects'
					? projects.map((p) => ({
							...
							title: p.name,
							icon: p.icon ?? Circle,
							href: resolve('/dashboard/workspaces/[slug]/projects/[projectId]', {
								slug,
								projectId: p.id
							}),
							matchPrefix: true,
	...
	const userProfile = AuthFacade.useTanstack().userProfile();
</script>
...
<Sidebar.Footer>
	<Sidebar.Menu class="flex flex-col gap-5 ">
		{#if isCompact}
			<Sidebar.MenuItem>
				<Sidebar.MenuButton
					onclick={() => goto(resolve('/dashboard/workspaces'))}
					class="group rounded-lg text-muted-foreground transition-all duration-200 hover:bg-muted"
				>
					<ArrowLeft
		...
		<Sidebar.MenuItem>
			<UserLogo
				id={userProfile.data?.value?.id ?? ''}
				name={userProfile.data?.value?.username ?? ''}
				size={isCompact ? 40 : 30}
			>
				{#snippet badge()}
					{#if isCompact}
