<script lang="ts">
	...
	const { mutateAsync: RegisterAsync } = AuthFacade.useTanstack().register();
	...
	async function handleSubmit(event: SubmitEvent) {
		...
		try {
			const { isFailure, errorMessage } = await RegisterAsync({
			...
			if (isFailure) {
				error = errorMessage || 'Unable to create your account.';
				return;
			}
			...
			redirectTimer = setTimeout(async () => {
				if (onSubmit) {
					...
					await goto(resolve('/dashboard/workspaces'));
				}
			}, 1500);
		} catch (err) {
